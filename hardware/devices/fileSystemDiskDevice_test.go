...
func Test_Mount1(t *testing.T) {
	...
	fsd.doMount(pkt)
	if pkt.IoStatus != ioPackets.IosInvalidPacket {
...
func Test_IO(t *testing.T) {
	...
	for blockId := hardware.BlockId(0); blockId < 1000; blockId++ {
		pkg.SerializeUint64IntoBuffer(uint64(blockId), buffer)
		pkt := &ioPackets.DiskIoPacket{
			Listener:   nil,
		...
		fsd.StartIo(pkt)
		if pkt.IoStatus != ioPackets.IosComplete {
	...
	for blockId := hardware.BlockId(0); blockId < 1000; blockId++ {
		...
		chk := pkg.DeserializeUint64FromBuffer(buffer)
