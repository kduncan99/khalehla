// khalehla Project
// Copyright © 2023-2025 by Kurt Duncan, common.BearSnake LLC
// All Rights Reserved
...
func Test_EX_Basic(t *testing.T) {
	...
	checkRegister(t, engine, common.X5, 0_000001_000005)
	checkRegister(t, engine, common.R3, 0_123456)
}
...
var exExtendedMode = []*tasm.SourceItem{
	...
	lxiSourceItemU(jU, regX5, 0, 01),
	lxmSourceItemU(jU, regX5, 0, 04),
	exSourceItemHIBRef(regX5, 1, 0, common.B0, "target"),
	iarSourceItem(0),
...
var exExtendedModeCascade = []*tasm.SourceItem{
	...
	segSourceItem(0),
	exSourceItemHIBRef(0, 0, 0, common.B4, "target1"),
	iarSourceItem(0),

	segSourceItem(04),
	labelSourceItem("target1"),
	exSourceItemHIBRef(0, 0, 0, common.B4, "target2"),
	labelSourceItem("target2"),
	exSourceItemHIBRef(0, 0, 0, common.B4, "target3"),
	labelSourceItem("target3"),
	exSourceItemHIBRef(0, 0, 0, common.B5, "target4"),

	segSourceItem(05),
	labelSourceItem("target4"),
	lrSourceItemHIBRef(jH1, regR7, 0, 0, 0, common.B2, "data"),
}
...
func Test_EX_ExtendedCascade(t *testing.T) {
	...
	engine := ute.GetEngine()
	checkStoppedReason(t, engine, InitiateAutoRecoveryStop, 0)
	checkRegister(t, engine, common.R7, 0_123456)
}
...
var exrExtendedMode = []*tasm.SourceItem{
	...
	exrSourceItemHIBRef(regX7, 0, 0, common.B2, "target"),
	iarSourceItem(0),
	...
	dataSourceItem([]uint64{0}),
	dataSourceItem([]uint64{0}),
	sasSourceItemHIBRef(jH1, regX8, 1, 0, common.B3, "data"),
...
func Test_EXR_Extended(t *testing.T) {
	...
	engine := ute.GetEngine()
	checkStoppedReason(t, engine, InitiateAutoRecoveryStop, 0)
	checkRegister(t, engine, common.X8, 0_000001_000010)
...
func Test_EXR_ExtendedInvalidInstruction(t *testing.T) {
	...
	engine := ute.GetEngine()
	checkInterrupt(t, engine, common.InvalidInstructionInterruptClass)
}
...
func Test_EXR_ExtendedTZ(t *testing.T) {
	...
	engine := ute.GetEngine()
	checkStoppedReason(t, engine, InitiateAutoRecoveryStop, 0)
	checkRegister(t, engine, common.R1, 013)
}
...
func Test_DCB_ExtendedTest(t *testing.T) {
	...
	engine := ute.GetEngine()
	checkStoppedReason(t, engine, InitiateAutoRecoveryStop, 0)
	checkRegister(t, engine, common.A3, 46)
	checkRegister(t, engine, common.A4, 36)
	checkRegister(t, engine, common.A5, 1)
}
...
var rngbExtendedMode = []*tasm.SourceItem{
	...
	segSourceItem(0),
	rngbSourceItemHIBRef(0, 0, 0, common.B2, "data"),
	iarSourceItem(0),
}
...
var rngiExtendedMode = []*tasm.SourceItem{
	...
	segSourceItem(0),
	rngiSourceItemHIBRef(0, 0, 0, common.B2, "data"),
	iarSourceItem(0),
}
