...
const L0BDTBaseRegister = pkg2.B16
const ICSBaseRegister = pkg2.B26
const ICSIndexRegister = pkg2.EX1
const RCSBaseRegister = pkg2.B25
const RCSIndexRegister = pkg2.EX0
...
type GetOperandResult struct {
	...
	sourceAbsoluteAddress   *pkg2.AbsoluteAddress
	interrupt               pkg2.Interrupt
}
...
func (gor *GetOperandResult) GetString() string {
	return fmt.Sprintf(
		...
		pkg2.GetInterruptString(gor.interrupt))
}
...
func (or *ConsecutiveOperandsResult) GetString() string {
	return fmt.Sprintf(
		...
		pkg2.GetInterruptString(or.interrupt))
}
...
type InstructionEngine struct {
	...
	mainStorage  *pkg2.MainStorage // must be set externally
	storageLocks *StorageLocks     // must be set externally

	activeBaseTable           [16]*ipEngine.ActiveBaseTableEntry // [0] is unused
	activityStatePacket       *pkg2.ActivityStatePacket
	cachedInstructionHandler  func(*InstructionEngine) (completed bool)
	baseRegisters             [32]*pkg2.BaseRegister
	baseRegisterIndexForFetch uint // only applies to basic mode - if 0, it is not valid; otherwise it is 12:15
	generalRegisterSet        *pkg2.GeneralRegisterSet
	...
	breakpointAddress *pkg2.AbsoluteAddress
	breakpointHalt    bool
...
func NewEngine(name string, mainStorage *pkg2.MainStorage, storageLocks *StorageLocks) *InstructionEngine {
	e := &InstructionEngine{}
	e.name = name
...
func (e *InstructionEngine) Clear() {
	...
	for ax := 0; ax < 16; ax++ {
		e.activeBaseTable[ax] = ipEngine.NewActiveBaseTableEntryFromComposite(0)
	}

	for bx := 0; bx < 32; bx++ {
		e.baseRegisters[bx] = pkg2.NewVoidBaseRegister()
	}
	e.baseRegisterIndexForFetch = 0

	e.generalRegisterSet = pkg2.NewGeneralRegisterSet()
	e.activityStatePacket = pkg2.NewActivityStatePacket()
...
func (e *InstructionEngine) GetActiveBaseTableEntry(index uint64) *ipEngine.ActiveBaseTableEntry {
	return e.activeBaseTable[index]
}

// GetBaseRegister retrieves a pointer to the indicated base register
func (e *InstructionEngine) GetBaseRegister(index uint64) *pkg2.BaseRegister {
...
func (e *InstructionEngine) GetConsecutiveOperands(grsCheck bool, count uint64, forUpdate bool) (result ConsecutiveOperandsResult) {
	...
	if (grsCheck) &&
		...
		for ox := uint64(0); ox < count; ox++ {
			if grsIndex == 0200 {
				result.interrupt = pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationGRS, false)
				return
			}

			if !e.isGRSAccessAllowed(grsIndex, dr.GetProcessorPrivilege(), false) {
				result.interrupt = pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationReadAccess, false)
				return
			}
...
func (e *InstructionEngine) GetCurrentInstruction() *pkg2.InstructionWord {
	return e.activityStatePacket.GetCurrentInstruction()
}

func (e *InstructionEngine) GetDesignatorRegister() *pkg2.DesignatorRegister {
...
func (e *InstructionEngine) GetExecOrUserARegister(registerIndex uint64) *pkg2.Word36 {
...
func (e *InstructionEngine) GetExecOrUserARegisterIndex(registerIndex uint64) uint64 {
	if e.activityStatePacket.GetDesignatorRegister().IsExecRegisterSetSelected() {
		return pkg2.EA0 + registerIndex
	} else {
		return pkg2.A0 + registerIndex
	}
}

// GetExecOrUserRRegister retrieves either the ER{index} or R{index} register
// depending upon the setting of designator register ExecRegisterSetSelected
func (e *InstructionEngine) GetExecOrUserRRegister(registerIndex uint64) *pkg2.Word36 {
...
func (e *InstructionEngine) GetExecOrUserRRegisterIndex(registerIndex uint64) uint64 {
	if e.activityStatePacket.GetDesignatorRegister().IsExecRegisterSetSelected() {
		return pkg2.ER0 + registerIndex
	} else {
		return pkg2.R0 + registerIndex
	}
}
...
func (e *InstructionEngine) GetExecOrUserXRegisterIndex(registerIndex uint64) uint64 {
	if e.activityStatePacket.GetDesignatorRegister().IsExecRegisterSetSelected() {
		return pkg2.EX0 + registerIndex
	} else {
		return pkg2.X0 + registerIndex
	}
}

// GetGeneralRegisterSet retrieves a pointer to the GRS
func (e *InstructionEngine) GetGeneralRegisterSet() *pkg2.GeneralRegisterSet {
...
func (e *InstructionEngine) GetImmediateOperand() (operand uint64, interrupt pkg2.Interrupt) {
	operand = 0
	interrupt = nil
...
// Returns complete == false if we are in the middle of resolving addresses.
func (e *InstructionEngine) GetJumpOperand() (operand uint64, flip31 bool, completed bool, interrupt pkg2.Interrupt) {
	flip31 = false
...
func (e *InstructionEngine) GetOperand(
	...
	if allowImm && ((jField == pkg2.JFieldU) || (jField == pkg2.JFieldXU)) {
	...
	if (grsCheck) && (basicMode || (result.sourceBaseRegisterIndex == 0)) && (result.sourceRelativeAddress < 0200) {
		...
		if !e.isGRSAccessAllowed(result.sourceRelativeAddress, privilege, false) {
			result.interrupt = pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationReadAccess, true)
			return
		}
		...
		if grsDest || !allowPartial {
			...
			result.operand = pkg2.ExtractPartialWord(grs.GetRegister(result.sourceRelativeAddress).GetW(), jField, qWordMode)
		}
		...
		if allowPartial {
			qWordMode := dReg.IsQuarterWordModeEnabled()
			result.operand = pkg2.ExtractPartialWord(result.source.GetW(), jField, qWordMode)
		} else {
...
func (e *InstructionEngine) GetProgramAddressRegister() *pkg2.ProgramAddressRegister {
...
func (e *InstructionEngine) IgnoreOperand() (complete bool, interrupt pkg2.Interrupt) {
	var relAddr uint64
...
func (e *InstructionEngine) PostInterrupt(i pkg2.Interrupt) {
	fmt.Printf("===Posting %s\n", pkg2.GetInterruptString(i)) // TODO remove later
...
func (e *InstructionEngine) SetBaseRegister(brIndex uint64, register *pkg2.BaseRegister) {
...
func (e *InstructionEngine) Stop(reason StopReason, detail pkg2.Word36) {
...
func (e *InstructionEngine) StoreConsecutiveOperands(
	grsCheck bool,
	operands []uint64) (complete bool, interrupt pkg2.Interrupt) {
	...
	if (grsCheck) && (basicMode || (brx == 0)) && (relAddr < 0200) {
		...
		for ox := uint64(0); ox < count; ox++ {
			if grsIndex == 0200 {
				interrupt = pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationGRS, false)
				return
			}

			if !e.isGRSAccessAllowed(grsIndex, dr.GetProcessorPrivilege(), true) {
				interrupt = pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationReadAccess, false)
				return
			}
		...
		var absAddr *pkg2.AbsoluteAddress
		_, absAddr, interrupt = e.translateAddress(brx, relAddr)
		if interrupt != nil {
			return
		}

		var dest []pkg2.Word36
...
func (e *InstructionEngine) StoreOperand(
	...
	checkImmediate bool,
	allowPartial bool,
	operand uint64) (complete bool, interrupt pkg2.Interrupt) {
	...
	if (grsCheck) && (basicMode || (brx == 0)) && (relAddr < 0200) {
		...
		if !e.isGRSAccessAllowed(relAddr, privilege, true) {
			interrupt = pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationWriteAccess, false)
			return
		}
		...
		if !grsSource && allowPartial {
			...
			newValue := pkg2.InjectPartialWord(originalValue, operand, jField, qWordMode)
...
func (e *InstructionEngine) checkAccessibility(
	bReg *pkg2.BaseRegister,
	fetchFlag bool,
	readFlag bool,
	writeFlag bool,
	accessKey *pkg2.AccessKey) pkg2.Interrupt {
	...
	if e.activityStatePacket.GetDesignatorRegister().IsBasicModeEnabled() && fetchFlag && !perms.CanEnter() {
		...
		return pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationReadAccess, fetchFlag)
	} else if readFlag && !perms.CanRead() {
		return pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationReadAccess, fetchFlag)
	} else if writeFlag && !perms.CanWrite() {
		return pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationWriteAccess, fetchFlag)
	}
...
func (e *InstructionEngine) checkAccessLimitsForAddress(
	...
	baseRegisterIndex uint,
	relativeAddress uint64,
	fetchFlag bool) (interrupt pkg2.Interrupt) {

	interrupt = nil
	if fetchFlag && relativeAddress < 0200 {
		if basicMode || baseRegisterIndex == 0 {
			interrupt = pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationStorageLimits, true)
			return
		}
	}
	...
	bReg := e.baseRegisters[baseRegisterIndex]
	if bReg.IsVoid() {
		return pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationStorageLimits, fetchFlag)
	}
...
func (e *InstructionEngine) checkAccessLimitsRange(
	...
	if (relativeAddress < bDesc.GetLowerLimitNormalized()) ||
		...
		return pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationStorageLimits, false)
	}

	return e.checkAccessibility(bReg, false, readFlag, writeFlag, accessKey)
}

func (e *InstructionEngine) checkBreakpoint(comparison BreakpointComparison, absAddr *pkg2.AbsoluteAddress) (found bool, interrupt pkg2.Interrupt) {
	found = false
	interrupt = nil
	...
	if e.breakpointAddress != nil && e.breakpointAddress.Equals(absAddr) {
		if (comparison == BreakpointFetch && e.breakpointFetch) ||
			...
			if e.breakpointHalt {
				...
				interrupt = pkg2.NewBreakpointInterrupt()
			}
		}
	}
...
func (e *InstructionEngine) checkBreakpointRange(
	...
	absAddr *pkg2.AbsoluteAddress,
	count uint64) (found bool, interrupt pkg2.Interrupt) {
...
func (e *InstructionEngine) executeCurrentInstruction() (completed bool) {
	...
	if e.cachedInstructionHandler == nil {
		...
		if !found {
			...
			e.PostInterrupt(pkg2.NewInvalidInstructionInterrupt(pkg2.InvalidInstructionBadFunctionCode))
			e.SetInstructionPoint(BetweenInstructions)
			return false
		}
	}
...
func (e *InstructionEngine) fetchInstructionWord() bool {
	...
	if basicMode {
		...
		if e.baseRegisterIndexForFetch == 0 {
			...
			if brx == 0 {
				interrupt := pkg2.NewReferenceViolationInterrupt(pkg2.ReferenceViolationStorageLimits, false)
				e.PostInterrupt(interrupt)
				return false
			}
	...
	iw := pkg2.InstructionWord(bReg.GetStorage()[pcOffset])
	asp := e.activityStatePacket
...
func (e *InstructionEngine) findBankDescriptor(bankLevel uint64, bankDescriptorIndex uint64) (*pkg2.BankDescriptor, bool) {
	...
	if e.baseRegisters[bdRegIndex].IsVoid() {
		e.PostInterrupt(pkg2.NewAddressingExceptionInterrupt(pkg2.AddressingExceptionFatal, bankLevel, bankDescriptorIndex))
		return nil, false
	}
	...
	bd := pkg2.NewBankDescriptorFromStorage(bdStorage[bdTableOffset : bdTableOffset+8])
	return bd, true
}
...
func (e *InstructionEngine) findBaseRegisterIndexBasicMode(relAddr uint64) (baseRegisterIndex uint, interrupt pkg2.Interrupt) {
	interrupt = nil
...
func (e *InstructionEngine) findBaseRegisterIndex(relAddr uint64) (baseRegisterIndex uint, interrupt pkg2.Interrupt) {
	baseRegisterIndex = 0
	interrupt = nil
...
func (e *InstructionEngine) isReadAllowed(bReg *pkg2.BaseRegister) bool {
...
func (e *InstructionEngine) isWithinLimits(bReg *pkg2.BaseRegister, offset uint64) bool {
	return !bReg.IsVoid() &&
...
func (e *InstructionEngine) resolveRelativeAddress(useU bool) (relAddr uint64, complete bool, interrupt pkg2.Interrupt) {
	relAddr = 0
	complete = false
	interrupt = nil
...
func (e *InstructionEngine) translateAddress(
	baseRegisterIndex uint,
	relativeAddress uint64) (virAddr hardware.VirtualAddress, absAddr *pkg2.AbsoluteAddress, interrupt pkg2.Interrupt) {
	...
	bReg := e.baseRegisters[baseRegisterIndex]
	if bReg.IsVoid() {
		interrupt = pkg2.NewAddressingExceptionInterrupt(pkg2.AddressingExceptionFatal, level, bdi)
		return
	}

	bDesc := bReg.GetBankDescriptor()
	offset += relativeAddress - bDesc.GetLowerLimitNormalized()
	if bDesc.GetBankType() == pkg2.BasicModeBankDescriptor {
		virAddr = hardware.TranslateToBasicMode(level, bdi, offset)
	} else if bDesc.GetBankType() == pkg2.ExtendedModeBankDescriptor {
	...
	absAddr = pkg2.NewAbsoluteAddress(bDesc.GetBaseAddress().GetSegment(), offset)
