...
func NewInstructionProcessor(name string, mainStorage *pkg2.MainStorage, storageLocks *StorageLocks) *InstructionProcessor {
...
func (p *InstructionProcessor) GetStopDetail() pkg2.Word36 {
...
// IsStopped indicates whether the ipEngineOld is stopped
...
func (p *InstructionProcessor) handleInterrupt(i pkg2.Interrupt) {
	...
	if i.GetClass() == pkg2.HardwareCheckInterruptClass &&
	...
	icsXReg := common.IndexRegister(grs.GetRegisterValue(ICSIndexRegister))
	...
	icsSlice[1] = pkg2.Word36(asp.GetDesignatorRegister().GetComposite())
	icsSlice[2] = asp.GetIndicatorKeyRegister().GetComposite()
	icsSlice[3] = asp.GetQuantumTimer()
	icsSlice[4] = pkg2.Word36(*asp.GetCurrentInstruction())
	...
	ipEngine.NewBankManipulatorForInterrupt(p.engine, i).process()
}

func (p *InstructionProcessor) isReadAllowed(bReg *pkg2.BaseRegister) bool {
...
func (p *InstructionProcessor) isWithinLimits(bReg *pkg2.BaseRegister, offset uint64) bool {
	return !bReg.IsVoid() &&
