...
func handleQual(pkt *handlerPacket) (facResult *facilitiesMgr.FacStatusResult, resultCode uint64) {
	...
	if len(pkt.pcs.operandFields) > 1 {
		klog.LogInfoF("CSIQual", "%v:Too many op fields '%v'", pkt.rce.RunId, pkt.pcs.originalStatement)
		goto syntaxError
	} else if len(pkt.pcs.operandFields) == 1 {
		if len(pkt.pcs.operandFields[0]) != 1 {
			klog.LogInfoF("CSIQual", "%v:Wrong # op subfields '%v'", pkt.rce.RunId, pkt.pcs.originalStatement)
			goto syntaxError
		} else {
			qualifier = pkt.pcs.operandFields[0][0]
			if !kexec.IsValidQualifier(qualifier) {
				klog.LogInfoF("CSIQual", "%v:Invalid qualifier '%v'", pkt.rce.RunId, pkt.pcs.originalStatement)
				goto syntaxError
			}
		}
	}
	...
	if optWord == 0 {
		...
		if len(qualifier) == 0 {
			klog.LogInfoF("CSIQual", "%v: Missing qualifier '%v'", pkt.rce.RunId, pkt.pcs.originalStatement)
		...
		if len(qualifier) > 0 {
			klog.LogInfoF("CSIQual", "%v: Should not specify qualifier with R option '%v'",
		...
		klog.LogInfoF("CSIQual", "%v: Conflicting options '%v'", pkt.rce.RunId, pkt.pcs.originalStatement)
		if pkt.sourceIsExecRequest {
			pkt.rce.PostContingency(012, 04, 040)
		}
