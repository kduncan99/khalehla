...
// MFDDescriptorFlags is a 12-bit field found in main item sector 0.
// It contains certain flags regarding a particular file.
type MFDDescriptorFlags struct {
	unloaded            bool
	backedUp            bool
...
func (df *MFDDescriptorFlags) Compose() uint64 {
	value := uint64(0)
	if df.unloaded {
		value |= 0_4000
	}
...
func (df *MFDDescriptorFlags) ExtractFrom(field uint64) {
