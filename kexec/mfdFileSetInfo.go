...
type MFDFileSetCycleInfo struct {
	ToBeCataloged bool
	ToBeDropped   bool
	AbsoluteCycle uint
}

type MFDFileSetInfo struct {
	Qualifier       string
	Filename        string
	ProjectId       string
	ReadKey         string
	WriteKey        string
	FileType        MFDFileType
	PlusOneExists   bool
	Count           uint
	MaxCycleRange   uint
	CurrentRange    uint
	HighestAbsolute uint
	CycleInfo       []MFDFileSetCycleInfo
}

// PopulateFromLeadItems populates the MFDFileSetInfo object from the Content of the
// given leadItem0 and (optional) leadItem1 sectors.
// If there is no leadItem1, that argument should be nil.
func (fsi *MFDFileSetInfo) PopulateFromLeadItems(leadItem0 []pkg.Word36, leadItem1 []pkg.Word36) {
	...
	fsi.FileType = NewMFDFileTypeFromField(leadItem0[011].GetS1())
	...
	fsi.CycleInfo = make([]MFDFileSetCycleInfo, fsi.MaxCycleRange)
	...
	for ax := 0; ax < int(fsi.MaxCycleRange); ax++ {
		...
		if link > 0 {
			fsi.CycleInfo[ax] = MFDFileSetCycleInfo{
