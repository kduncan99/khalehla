...
func main() {
	...
	logger.ClearLoggers()
	logger.RegisterLogger(logger.NewTimestampedFileLogger(logger.LevelAll, "kexec"))
	logger.SetGlobalLevel(logger.LevelAll)
	defer logger.Close()
	...
	channel := make(chan kexec2.StopCode)
	session := uint(0)
