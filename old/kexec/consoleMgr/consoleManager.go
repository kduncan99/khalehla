...
type readReplyTracker struct {
	trackerId    int
	message      *kexec2.ConsoleReadReplyMessage
	replyConsole *kexec2.ConsoleIdentifier // this is the console which is currently supposed to answer
	hasReply     bool
	isCanceled   bool
...
type ConsoleManager struct {
	...
	threadDone       bool
	threadStop       bool
	consoles         map[kexec2.ConsoleIdentifier]kexec.IConsole
	primaryConsole   kexec.IConsole
	primaryConsoleId kexec2.ConsoleIdentifier
	queuedReadOnly   []*kexec2.ConsoleReadOnlyMessage
...
func (mgr *ConsoleManager) Boot() error {
	...
	mgr.consoles = map[kexec2.ConsoleIdentifier]kexec.IConsole{
		mgr.primaryConsoleId: mgr.primaryConsole,
	}
	_ = mgr.primaryConsole.Reset()

	// clear the console queues
	mgr.queuedReadOnly = make([]*kexec2.ConsoleReadOnlyMessage, 0)
...
func (mgr *ConsoleManager) Initialize() error {
	logger.LogTrace("ConsMgr", "Initialize")
	mgr.consoles = make(map[kexec2.ConsoleIdentifier]kexec.IConsole)
	mgr.queuedReadOnly = make([]*kexec2.ConsoleReadOnlyMessage, 0)
	mgr.queuedReadReply = make(map[int]*readReplyTracker)

	mgr.primaryConsole = NewStandardConsole()
	mgr.primaryConsoleId = kexec2.ConsoleIdentifier(pkg.NewFromStringToFieldata("SYSCON", 1)[0])
...
func (mgr *ConsoleManager) SendReadOnlyMessage(message *kexec2.ConsoleReadOnlyMessage) {
	// Log it and put it in the RCE tail sheet (unless it is the Exec)
	logger.LogInfoF("ConsMgr", "Queueing %v*%v", message.Source.RunId, message.Text)
...
func (mgr *ConsoleManager) SendReadReplyMessage(message *kexec2.ConsoleReadReplyMessage) error {
	// LogF it and put it in the RCE tail sheet (unless it is the Exec)
	logger.LogInfoF("ConsMgr", "Queueing n-%v:%v", message.Source.RunId, message.Text)
	if !message.Source.IsExec() {
		message.Source.PostToTailSheet(message.Text)
	}
	...
	if tracker.isCanceled {
		logger.LogInfo("ConsMgr", "Read reply message was canceled")
...
func (mgr *ConsoleManager) checkForReadReplyMessages() bool {
	...
	for _, tracker := range mgr.queuedReadReply {
		...
		if tracker.replyConsole == nil && !tracker.retryLater {
			...
			var consoleId kexec2.ConsoleIdentifier
...
func (mgr *ConsoleManager) checkForSolicitedInput() bool {
	...
	for consId, cons := range mgr.consoles {
		...
		if err != nil {
			...
			if err != nil {
				logger.LogWarning("ConsMgr", "Received reply for a message we are not tracking")
				continue
			}

			tracker := mgr.queuedReadReply[trackerId]
			if tracker.hasReply {
				logger.LogWarning("ConsMgr", "Received reply for a message we already have a reply for")
				continue
			}
			...
			if mgr.exec.GetConfiguration().LogConsoleMessages && !tracker.message.DoNotLogReply {
				consw36 := pkg2.Word36(consId)
				logger.LogInfoF("ConsMgr", "ConsMgr:%v %v %v", consw36.ToStringAsFieldata(), msgId, *reply)
	...
	for consId, cons := range mgr.consoles {
		...
		if err != nil {
			...
			if mgr.exec.GetConfiguration().LogConsoleMessages {
				logger.LogInfoF("ConsMgr", "%v", *input)
			}
...
// call under lock
func (mgr *ConsoleManager) dropConsole(consoleId kexec2.ConsoleIdentifier) {
	consId := pkg2.Word36(consoleId)
	logger.LogInfoF("ConsMgr", "Deleting unresponsive console %v", consId.ToStringAsFieldata())
...
// call under lock
func (mgr *ConsoleManager) findTrackerFor(consoleId kexec2.ConsoleIdentifier, messageId int) (int, error) {
...
func (mgr *ConsoleManager) newReadReplyTracker(message *kexec2.ConsoleReadReplyMessage) *readReplyTracker {
...
func (mgr *ConsoleManager) Dump(dest io.Writer, indent string) {
	...
	for _, msg := range mgr.queuedReadOnly {
		...
		if msg.Routing != nil {
			consId := pkg2.Word36(*msg.Routing)
	...
	for tid, tracker := range mgr.queuedReadReply {
		...
		if tracker.replyConsole != nil {
			consId := pkg2.Word36(*tracker.replyConsole)
