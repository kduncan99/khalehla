...
import (
	...
	"khalehla/old/kexec/consoleMgr"
	"khalehla/old/kexec/mfdMgr"
...
type Exec struct {
	...
	runControlEntry *kexec2.RunControlEntry
	runControlTable map[string]*kexec2.RunControlEntry // indexed by runid

	jumpKeys []bool
	phase    kexec.ExecPhase
	stopCode kexec2.StopCode
	stopFlag bool
}
...
func (e *Exec) Boot(session uint, jumpKeys []bool, invokerChannel chan kexec2.StopCode) {
	e.jumpKeys = jumpKeys
	e.stopFlag = false
	...
	e.runControlEntry = kexec2.NewExecRunControlEntry(e.configuration.MasterAccountId)
	e.runControlTable = make(map[string]*kexec2.RunControlEntry)
...
func (e *Exec) GetRunControlEntry() *kexec2.RunControlEntry {
	return e.runControlEntry
}

func (e *Exec) GetStopCode() kexec2.StopCode {
	return e.stopCode
}
...
func (e *Exec) SendExecReadOnlyMessage(message string, routing *kexec2.ConsoleIdentifier) {
	consMsg := kexec2.ConsoleReadOnlyMessage{
...
func (e *Exec) SendExecReadReplyMessage(
	message string,
	maxReplyChars int,
	routing *kexec2.ConsoleIdentifier,
) (string, error) {
	consMsg := kexec2.ConsoleReadReplyMessage{
		Source:         e.runControlEntry,
		Routing:        routing,
		Text:           message,
		DoNotEmitRunId: true,
...
func (e *Exec) Stop(code kexec2.StopCode) {
	// TODO need to set contingency in the Exec RCE
	e.stopFlag = true
	e.stopCode = code
	if e.phase == kexec.ExecPhaseRunning {
		e.phase = kexec.ExecPhaseStopped
	} else {
		e.phase = kexec.ExecPhaseInitStopped
	}
}
