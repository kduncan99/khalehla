...
func (e *Exec) callCSI(statement string) uint64 {
	...
	if stat&0_400000_000000 != 0 {
		logger.LogErrorF("Exec", "Status %012o : %v", stat, statement)
	}
	return stat
}
...
func (e *Exec) loadSystemLibrary() {
	logger.LogTrace("Exec", "loadSystemLibrary")
	reelNumber := ""
	for !kexec2.IsValidReelNumber(reelNumber) {
		msg := "Enter reel number of system library tape"
		reelNumber, _ := e.SendExecReadReplyMessage(msg, 6, nil)
		if e.stopFlag {
			return
		}
		if kexec2.IsValidReelNumber(reelNumber) {
			break
		}
	...
	if e.stopFlag {
		logger.LogTrace("Exec", "loadSystemLibrary early exit")
		return
	}
	...
	if e.stopFlag {
		...
		logger.LogFatalF("Exec", "Cannot assign tape unit for library tape:%012o", stat)
		e.Stop(kexec2.StopFileAssignErrorOccurredDuringSystemInitialization)
		return
	}
	...
	if e.stopFlag {
		...
		logger.LogFatalF("Exec", "Cannot catalog SYS$*LIB$ file:%012o", stat)
		e.Stop(kexec2.StopFileAssignErrorOccurredDuringSystemInitialization)
		return
	}
	...
	if e.stopFlag {
		...
		logger.LogFatalF("Exec", "Cannot catalog SYS$*RUN$ file:%012o", stat)
		e.Stop(kexec2.StopFileAssignErrorOccurredDuringSystemInitialization)
		return
	}
	...
	if e.stopFlag {
		...
		logger.LogFatalF("Exec", "Cannot assign SYS$*RUN$ file:%012o", stat)
		e.Stop(kexec2.StopFileAssignErrorOccurredDuringSystemInitialization)
		return
	}
	...
	if e.stopFlag {
		...
		logger.LogFatalF("Exec", "Cannot catalog SYS$*RLIB$ file:%012o", stat)
		e.Stop(kexec2.StopFileAssignErrorOccurredDuringSystemInitialization)
		return
	}
	...
	if e.stopFlag {
		...
		logger.LogFatalF("Exec", "Cannot assign SYS$*RLIB$ file:%012o", stat)
		e.Stop(kexec2.StopFileAssignErrorOccurredDuringSystemInitialization)
		return
	}
	...
	logger.LogTrace("Exec", "loadSystemLibrary normal exit")
}
...
func (e *Exec) performInitialBoot() {
	if e.jumpKeys[kexec.JumpKey13Index] {
		...
		if e.stopFlag {
			...
			e.Stop(kexec2.StopConsoleResponseRequiresReboot)
		}
		...
		if e.stopFlag {
			...
			logger.LogFatalF("Exec", "Cannot catalog SYS$*DLOC$ file:%012o", stat)
			e.Stop(kexec2.StopFileAssignErrorOccurredDuringSystemInitialization)
			return
		}
