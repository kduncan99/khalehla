...
func (mgr *FacilitiesManager) canAccessFileCycle(
	...
	fileCycleInfo mfdMgr2.FileCycleInfo,
) bool {
	var match bool
...
func canCatalogFile(
	rce *kexec2.RunControlEntry,
	fileSetInfo *mfdMgr2.FileSetInfo,
...
func (mgr *FacilitiesManager) selectEquipmentModel(
	mnemonic string,
	fileSetInfo *mfdMgr2.FileSetInfo,
) ([]hardware2.NodeModel, config.EquipmentUsage, bool) {
	...
	models := make([]hardware2.NodeModel, 0)
	usage := entry.Usage
	for _, modelName := range entry.SelectableEquipment {
		model, ok := hardware2.NodeModelTable[modelName]
		if ok {
...
func (mgr *FacilitiesManager) assignWait(
	rce *kexec2.RunControlEntry,
	optionWord uint64,
	fsInfo *mfdMgr2.FixedFileCycleInfo, // nil if we are waiting on temporary file
	usage config.EquipmentUsage,
	facResult *FacStatusResult,
) (resultCode uint64) {
	logger.LogTraceF("FacMgr", "[%v] assignWait", rce.RunId)
	resultCode = 0
	...
	logger.LogTraceF("FacMgr", "[%v] returning %012o", rce.RunId, resultCode)
	return
}
...
func (mgr *FacilitiesManager) assignCatalogedFixedFileToRun(
	...
	fileSpecification *kexec2.FileSpecification,
	optionWord uint64,
	fcInfo *mfdMgr2.FixedFileCycleInfo,
	facResult *FacStatusResult,
) (resultCode uint64) {
	resultCode = 0
	...
	mfdResult := mgr.exec.GetMFDManager().(*mfdMgr2.MFDManager).AccelerateFileCycle(fcInfo.FileCycleIdentifier)
	if mfdResult != mfdMgr.MFDSuccessful {
		mgr.exec.Stop(kexec2.StopFacilitiesComplex)
		return
	}
...
func (mgr *FacilitiesManager) assignCatalogedFile(
	...
	logger.LogTraceF("FacMgr", "assignCatalogedFile [%v]", rce.RunId)
	...
	if mfdResult == mfdMgr.MFDNotFound {
		logger.LogInfoF("FacMgr", "[%v] file not cataloged", rce.RunId)
	...
	if mfdResult == mfdMgr.MFDNotFound {
		logger.LogInfoF("FacMgr", "[%v] internal error", rce.RunId)
		facResult.PostMessage(kexec.FacStatusFileIsNotCataloged, nil)
		resultCode |= 0_400010_000000
		return
	}
	...
	*/
	...
	if fileSpecification.HasFileCycleSpecification() &&
		...
		*fileSpecification.FileCycleSpec.AbsoluteCycle != 0 {
		...
		facResult.PostMessage(kexec.FacStatusFileIsNotCataloged, nil)
		resultCode |= 0_400010_000000
		logger.LogTraceF("FacMgr", "[%v] file does not exist", rce.RunId)
		return
	}
	...
	for _, facItem := range rce.FacilityItems {
		if fileSpecification.Filename == facItem.GetFilename() &&
			...
			logger.LogTraceF("FacMgr", "[%v] already assigned", rce.RunId)
			return
		}
	}
...
func (mgr *FacilitiesManager) assignTemporaryFile(
	...
	optionWord uint64,
	operandFields [][]string,
	models []hardware2.NodeModel,
	usage config.EquipmentUsage, // TODO does this account for unit assigns? I'm not sure...
	facResult *FacStatusResult,
) (resultCode uint64) {
	logger.LogTraceF("FacMgr", "[%v] assignTemporaryFile", rce.RunId)
	defer logger.LogTraceF("FacMgr", "[%v] assignTemporaryFile result %012o", rce.RunId, resultCode)
...
func (mgr *FacilitiesManager) assignToBeCatalogedFile(
	...
	logger.LogTraceF("FacMgr", "assignToBeCatalogedFile [%v]", rce.RunId)
...
func (mgr *FacilitiesManager) attachUnit(
	...
	logger.LogTraceF("FacMgr", "attachUnit(nodeId=%v runid=%s)", nodeId, rce.RunId)
...
func (mgr *FacilitiesManager) catalogCommon(
	...
	var granularity kexec2.Granularity
	...
	mm := exec.GetMFDManager().(*mfdMgr2.MFDManager)
	if fileSetInfo == nil {
		fsIdent, result := mm.CreateFileSet(
			mfdMgr2.FileTypeFixed,
		...
		if result == mfdMgr.MFDInternalError {
			...
			logger.LogFatal("FacMgr", "MFD failed to create file set")
			exec.Stop(kexec2.StopFacilitiesComplex)
			return false
		}
	...
	descriptorFlags := mfdMgr2.DescriptorFlags{
	...
	inhibitFlags := mfdMgr2.InhibitFlags{
	...
	for retry {
		...
		case mfdMgr.MFDDropOldestCycleRequired:
			...
			if result == mfdMgr.MFDInternalError {
				...
				logger.LogFatal("FacMGR", "Cannot delete oldest file cycle")
				mgr.exec.Stop(kexec2.StopFacilitiesComplex)
...
func (mgr *FacilitiesManager) detachUnit(
	...
	logger.LogTraceF("FacMgr", "detachUnit(nodeId=%v)", nodeId)
	rce, ok := mgr.attachments[nodeId]
	if ok {
		if force {
			logger.LogInfoF("FacMgr", "detachUnit forcing removal from run %s", rce.RunId)
