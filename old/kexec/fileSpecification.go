...
func (fs *FileSpecification) parseQualifierAndFilename(p *Parser) (fsCode FacStatusCode, ok bool) {
	...
	for !p.IsAtEnd() {
		...
		if !fs.HasAsterisk && IsValidQualifierChar(ch) {
			if len(fs.Qualifier) == 12 {
				logger.LogTrace("Exec", "Parse:qualifier too long")
		...
		if fs.HasAsterisk && IsValidFilenameChar(ch) {
			if len(fs.Filename) == 12 {
				logger.LogTrace("Exec", "Parse:filename too long")
				return FacStatusSyntaxErrorInImage, false
			} else {
		...
		if ch == '*' {
			if fs.HasAsterisk {
				logger.LogTrace("Exec", "Parse:multiple '*'")
				return FacStatusSyntaxErrorInImage, false
			} else {
				fs.HasAsterisk = true
			}
	...
	if len(fs.Filename) == 0 {
		logger.LogTrace("Exec", "Parse:filename required")
...
func (fs *FileSpecification) parseAbsoluteCycle(p *Parser) (found bool, fsCode FacStatusCode, ok bool) {
	...
	if err != nil || !found {
		logger.LogTrace("Exec", "Parse:abs cycle error")
	...
	if value < 1 || value > 999 {
		logger.LogTrace("Exec", "Parse:Illegal abs cycle")
	...
	p.SkipSpaces()
	if !p.ParseSpecificCharacter(')') {
		logger.LogTrace("Exec", "Parse:Abs cycle missing ')'")
		fsCode = FacStatusIllegalValueForFCycle
		ok = false
		return
	}
...
func (fs *FileSpecification) parseRelativeCycle(p *Parser) (found bool, fsCode FacStatusCode, ok bool) {
	...
	value, found, err := p.ParseUnsignedInteger()
	if err != nil || !found {
		logger.LogTrace("Exec", "Parse:Bad rel cycle")
		fsCode = FacStatusSyntaxErrorInImage
		ok = false
		return
	}
	...
	p.SkipSpaces()
	if !p.ParseSpecificCharacter(')') {
		logger.LogTrace("Exec", "Parse:Rel cycle missing ')'")
		fsCode = FacStatusSyntaxErrorInImage
		ok = false
		return
	}
	...
	if pos {
		if value == 1 {
			...
			logger.LogTrace("Exec", "Parse:Illegal rel cycle")
			fsCode = FacStatusIllegalValueForFCycle
			ok = false
			return
		}
...
func (fs *FileSpecification) parseKey(p *Parser) (key string, err error) {
	...
	if !IsValidReadWriteKey(key) {
		logger.LogTrace("Exec", "Parse:Invalid key")
...
func ParseFileSpecification(p *Parser) (fs *FileSpecification, fsCode FacStatusCode, ok bool) {
	logger.LogTraceF("Exec", "ParseFileSpecification '%v'", p.text)
	...
	if !ok {
		logger.LogTrace("Exec", "Parse:Failed parseQualifierAndFilename")
		return
	}

	fsCode, ok = fs.parseCycle(p)
	if !ok {
		logger.LogTrace("Exec", "Parse:Failed parseCycle")
		return
	}

	fsCode, ok = fs.parseKeys(p)
	if !ok {
		logger.LogTrace("Exec", "Parse:Failed parseKeys")
		return
	}
