...
func NewPREPKeyinHandler(exec kexec.IExec, source kexec2.ConsoleIdentifier, options string, arguments string) IKeyinHandler {
...
func (kh *PREPKeyinHandler) CheckSyntax() bool {
	...
	if !kexec2.IsValidNodeName(kh.deviceName) || !hardware2.IsValidPackName(kh.packName) {
		return false
	}
...
func (kh *PREPKeyinHandler) process() {
	...
	if attr.GetNodeCategoryType() != hardware.NodeCategoryDevice ||
		attr.GetNodeDeviceType() != hardware2.NodeDeviceDisk {
		str := fmt.Sprintf("%v is not a disk device", kh.deviceName)
		kh.exec.SendExecReadOnlyMessage(str, &kh.source)
		return
	}

	if !hardware2.IsValidPrepFactor(hardware.PrepFactor(kh.prepFactor)) {
	...
	ddi := nodeInfo.(*nodeMgr2.DiskDeviceInfo)
	...
	ioStat := kh.io(cp)
	if ioStat != ioPackets2.IosComplete {
