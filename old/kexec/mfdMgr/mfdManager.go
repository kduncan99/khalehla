...
type MFDManager struct {
	...
	mutex                      sync.Mutex
	threadDone                 bool
	mfdFileMainItem0Address    kexec2.MFDRelativeAddress                               // MFD address of MFD$$ main file item 0
	cachedTracks               map[kexec2.MFDRelativeAddress][]pkg.Word36              // key is MFD addr of first sector in track
	dirtyBlocks                map[kexec2.MFDRelativeAddress]bool                      // MFD addresses of blocks containing dirty sectors
	freeMFDSectors             map[kexec2.MFDRelativeAddress]bool                      // MFD addresses of existing but unused MFD sectors
	fixedPackDescriptors       map[kexec2.LDATIndex]*packDescriptor                    // packDescriptors of all known fixed packs
	fileLeadItemLookupTable    map[string]kexec2.MFDRelativeAddress                    // MFD address of lead item 0 of all cataloged files
	acceleratedFileAllocations map[kexec2.MFDRelativeAddress]*kexec2.FileAllocationSet // key is main item sector 0 address of file
}

func NewMFDManager(exec kexec.IExec) *MFDManager {
	return &MFDManager{
		exec:                       exec,
		cachedTracks:               make(map[kexec2.MFDRelativeAddress][]pkg.Word36),
		dirtyBlocks:                make(map[kexec2.MFDRelativeAddress]bool),
		freeMFDSectors:             make(map[kexec2.MFDRelativeAddress]bool),
		fixedPackDescriptors:       make(map[kexec2.LDATIndex]*packDescriptor),
		fileLeadItemLookupTable:    make(map[string]kexec2.MFDRelativeAddress),
		acceleratedFileAllocations: make(map[kexec2.MFDRelativeAddress]*kexec2.FileAllocationSet),
	}
}
...
func (mgr *MFDManager) Boot() error {
	...
	// reset tables
	mgr.cachedTracks = make(map[kexec2.MFDRelativeAddress][]pkg.Word36)
	mgr.dirtyBlocks = make(map[kexec2.MFDRelativeAddress]bool)
	mgr.freeMFDSectors = make(map[kexec2.MFDRelativeAddress]bool)
	mgr.fixedPackDescriptors = make(map[kexec2.LDATIndex]*packDescriptor)
	mgr.fileLeadItemLookupTable = make(map[string]kexec2.MFDRelativeAddress)
	mgr.acceleratedFileAllocations = make(map[kexec2.MFDRelativeAddress]*kexec2.FileAllocationSet)
