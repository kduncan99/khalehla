...
func (mgr *MFDManager) AccelerateFileCycle(
	...
	logger.LogTraceF("MFDMgr", "AccelerateFileCycle %012o", fcIdentifier)
	mgr.mutex.Lock()
	defer mgr.mutex.Unlock()

	mainItem0Addr := kexec2.MFDRelativeAddress(fcIdentifier)
...
func (mgr *MFDManager) CreateFixedFileCycle(
	...
	leadItem0Addr := kexec2.MFDRelativeAddress(fsIdentifier)
...
func (mgr *MFDManager) GetFileCycleInfo(
	...
	mainItem0, err := mgr.getMFDSector(kexec2.MFDRelativeAddress(fcIdentifier))
	if err != nil {
		mfdResult = MFDNotFound
		return
	}

	mainItem1Address := kexec2.MFDRelativeAddress(mainItem0[015].GetW()) & 0_007777_777777
	...
	for link&0_400000_000000 == 0 {
		mi, err := mgr.getMFDSector(kexec2.MFDRelativeAddress(link & 0_007777_777777))
		if err != nil {
			mfdResult = MFDInternalError
			return
		}
...
func (mgr *MFDManager) InitializeMassStorage() {
	...
	disks := make([]*nodeMgr2.DiskDeviceInfo, 0)
	fm := mgr.exec.GetFacilitiesManager()
	nm := mgr.exec.GetNodeManager().(*nodeMgr2.NodeManager)
	for _, dInfo := range nm.GetDeviceInfos() {
		if dInfo.GetNodeDeviceType() == hardware2.NodeDeviceDisk {
			disks = append(disks, dInfo.(*nodeMgr2.DiskDeviceInfo))
		}
	}
	...
	fixedDisks := make(map[*nodeMgr2.DiskDeviceInfo]*kexec2.DiskAttributes)
	removableDisks := make(map[*nodeMgr2.DiskDeviceInfo]*kexec2.DiskAttributes)
