...
type PackFreeSpaceTable struct {
	...
	Content  []*kexec2.TrackRegion
}

func NewPackFreeSpaceTable(capacity hardware.TrackCount) *PackFreeSpaceTable {
	fst := &PackFreeSpaceTable{}
	fst.Capacity = capacity
	fsr := kexec2.NewTrackRegion(0, capacity)
	fst.Content = []*kexec2.TrackRegion{fsr}
	return fst
}

func (fst *PackFreeSpaceTable) appendEntry(region *kexec2.TrackRegion) {
	fst.Content = append(fst.Content, region)
}

func (fst *PackFreeSpaceTable) prependEntry(region *kexec2.TrackRegion) {
	temp := []*kexec2.TrackRegion{region}
	temp = append(temp, fst.Content...)
	fst.Content = temp
}

func (fst *PackFreeSpaceTable) removeEntryAt(index int) {
	temp := make([]*kexec2.TrackRegion, 0)
...
func (fst *PackFreeSpaceTable) spliceNewEntryAt(region *kexec2.TrackRegion, index int) {
	temp := make([]*kexec2.TrackRegion, 0)
	temp = append(temp, fst.Content[:index]...)
...
func (fst *PackFreeSpaceTable) AllocateTrack() (kexec2.MFDTrackId, error) {
	// quick check...
	...
	for _, region := range fst.Content {
		if region.TrackCount == 1 {
			...
			return kexec2.MFDTrackId(trackId), nil
		}
	}

	// just use the next available
	region := fst.Content[0]
	trackId := kexec2.MFDTrackId(region.TrackId)
	fst.MarkTrackRegionUnallocated(region.TrackId, region.TrackCount)
	return trackId, nil
}
...
func (fst *PackFreeSpaceTable) AllocateTrackRegion(trackCount hardware.TrackCount) *kexec2.TrackRegion {
	if len(fst.Content) == 0 {
		return nil
	}
	...
	var largest *kexec2.TrackRegion
	var largestIndex int
	for rx, region := range fst.Content {
		if region.TrackCount > trackCount {
			result := kexec2.NewTrackRegion(region.TrackId, trackCount)
...
func (fst *PackFreeSpaceTable) MarkTrackRegionAllocated(
	ldatIndex kexec2.LDATIndex, // only for logging
	trackId hardware.TrackId,
	trackCount hardware.TrackCount,
) bool {
	if trackCount == 0 {
	...
	for fx, fsRegion := range fst.Content {
		...
		if trackId >= fsRegion.TrackId && trackId <= entryLimit {
			...
			newRegion := kexec2.NewTrackRegion(entryLimit, hardware.TrackCount(entryLimit-reqTrackLimit))
...
func (fst *PackFreeSpaceTable) MarkTrackRegionUnallocated(
	...
	for fx, fsRegion := range fst.Content {
		...
		if trackId < fsRegion.TrackId {
			re := kexec2.NewTrackRegion(trackId, trackCount)
			fst.appendEntry(re)
			return true
		}
	}
