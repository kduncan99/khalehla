...
func (mgr *NodeManager) Initialize() error {
	logger.LogTrace("NodeMgr", "Initialized")
	...
	for _, cInfo := range chInfos {
		cInfo.CreateNode()
		logger.LogInfoF("NodeMgr", "Created channel %v '%v'", cInfo.GetNodeIdentifier(), cInfo.GetNodeName())
	...
	for _, dInfo := range devInfos {
		dInfo.CreateNode()
		logger.LogInfoF("NodeMgr", "Created device %v '%v'", dInfo.GetNodeIdentifier(), dInfo.GetNodeName())
	...
	for cid, cInfo := range mgr.channelInfos {
		...
		case hardware2.NodeDeviceDisk:
			...
			for _, dInfo := range dchInfo.deviceInfos {
				...
				logger.LogInfoF("NodeMgr", "assigning %v to %v", dInfo.GetNodeName(), cInfo.GetNodeName())
				err := mgr.channelInfos[cid].GetChannel().AssignDevice(did, mgr.deviceInfos[did].GetDevice())
				if err != nil {
					logger.LogError("NodeMgr", err.Error())
					errors = true
				} else {
	...
	for _, dInfo := range mgr.deviceInfos {
		if !dInfo.IsAccessible() {
			logger.LogInfoF("NodeMgr", "%v is not accessible", dInfo.GetNodeName())
		}
	}

	if errors {
		logger.LogFatal("NodeMgr", "initialization errors exist")
		mgr.exec.Stop(kexec2.StopInitializationSystemConfigurationError)
...
func (mgr *NodeManager) RouteIo(cp *channels.ChannelProgram) {
	logger.LogTraceF("NodeMgr", "RouteIO %v", cp.GetString())
	...
	if err != nil {
		...
		mgr.exec.Stop(kexec2.StopErrorInSystemIOTable)
		return
	}
...
func (mgr *NodeManager) selectChannelForDevice(devInfo DeviceInfo) (ChannelInfo, error) {
	...
	logger.LogError("NodeMgr", "Cannot satisfy channel selection for accessible device")
	mgr.exec.Stop(kexec2.StopErrorAccessingFacilitiesDataStructure)
