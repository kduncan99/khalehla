...
func DoPrep(args []string) error {
	...
	label := make([]pkg2.Word36, prepFactor)
	...
	dirTrack := make([]pkg2.Word36, 1792)
...
func showLabelAndDirectory(
	...
	pkg2.DumpWord36Buffer(label[0:28], 7)
	if interpret {
	...
	dirBlock := make([]pkg2.Word36, prepFactor)
	...
	pkg2.DumpWord36Buffer(dirBlock[0:28], 7)

	var sector1 []pkg2.Word36
	if prepFactor == 28 {
		dirBlockId++
		sector1 = make([]pkg2.Word36, prepFactor)
	...
	pkg2.DumpWord36Buffer(sector1, 7)
...
func io(
	ch *channels2.DiskChannel,
	cp *channels.ChannelProgram,
) error {
	ch.StartIo(cp)
	for cp.IoStatus == ioPackets2.IosNotStarted || cp.IoStatus == ioPackets2.IosInProgress {
		time.Sleep(10 * time.Millisecond)
	}
	if cp.IoStatus != ioPackets2.IosComplete {
		fmt.Printf("IO error:%v status:%v\n", cp.GetString(), ioPackets2.IoStatusTable[cp.IoStatus])
