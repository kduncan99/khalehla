...
func (dev *FSTapeDevice) Close() error {
	...
	if ioPkt.GetIoStatus() != ioPackets2.IosComplete {
		return fmt.Errorf(ioPackets2.IoStatusTable[ioPkt.GetIoStatus()])
	}
...
func (dev *FSTapeDevice) ReadBlock() (data []pkg.Word36, eof bool, err error) {
	...
	dev.device.StartIo(ioPkt)
	if ioPkt.GetIoStatus() == ioPackets2.IosEndOfFile {
		return nil, true, nil
	} else if ioPkt.GetIoStatus() != ioPackets2.IosComplete {
		err = fmt.Errorf(ioPackets2.IoStatusTable[ioPkt.GetIoStatus()])
		return
	}
