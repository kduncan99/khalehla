...
func step1(bm *BankManipulator) bool {
	if bm.instructionType != InvalidInstruction {
		...
		if (bm.instructionType == LBUInstruction) &&
			(bm.processor.activityStatePacket.currentInstruction.GetA() < 2) {
			bm.processor.PostInterrupt(NewInvalidInstructionInterrupt(InvalidInstructionLBUUsesB0OrB1))
			return false
		}

		//	post AddressingExceptionInterrupt if IS is 3 for LxJ instructions
		if bm.isLXJInstruction && (bm.lxjInterfaceSpec == 3) {
			bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionInvalidISValue, 0, 0))
			return false
		}
	}
...
func step2(bm *BankManipulator) bool {
	if bm.instructionType == CALLInstruction {
		par := bm.processor.activityStatePacket.programAddressRegister
		...
		dr := bm.processor.activityStatePacket.designatorRegister
		...
		abte := bm.processor.GetActiveBaseTableEntry(abtx)
...
func step3(bm *BankManipulator) bool {
	if bm.interrupt != nil {
		...
		bReg := bm.processor.GetBaseRegister(L0BDTBaseRegister)
		if bReg.voidFlag {
			bm.processor.Stop(L0BaseRegisterInvalidStop, 0)
			return false
		}

		//  intOffset is the offset from the start of the level 0 BDT, to the vector we're interested in.
		bdtLevel0 := bm.processor.GetBaseRegister(L0BDTBaseRegister).storage
		intOffset := bm.interrupt.GetClass()
		if intOffset >= uint(len(bdtLevel0)) {
			bm.processor.Stop(InterruptHandlerOffsetOutOfRangeStop, 0)
			return false
		}
		...
		rcsBReg := bm.processor.GetBaseRegister(RCSBaseRegister)
		if rcsBReg.voidFlag {
			i := NewRCSGenericStackUnderOverflowInterrupt(RCSGenericStackOverflow, RCSBaseRegister, 0)
			bm.processor.PostInterrupt(i)
			return false
		}

		rcsXReg := bm.processor.GetExecOrUserXRegister(RCSIndexRegister)
		...
		bmSpec := bm.processor.GetExecOrUserXRegister(bm.lxjXRegisterIndex).GetW()
...
func step4(bm *BankManipulator) bool {
	if (bm.sourceBankLevel == 0) &&
		...
		if bm.interrupt != nil {
			detail := types.Word36((bm.sourceBankLevel << 15) | (bm.sourceBankDescriptorIndex))
			bm.processor.Stop(InterruptHandlerInvalidLevelBDIStop, detail)
			return false
		} else {
			i := NewAddressingExceptionInterrupt(AddressingExceptionInvalidSourceLBDI, bm.sourceBankLevel, bm.sourceBankDescriptorIndex)
			bm.processor.PostInterrupt(i)
			return false
		}
	}
...
func step5(bm *BankManipulator) bool {
	if (bm.sourceBankLevel == 0) && (bm.sourceBankDescriptorIndex == 0) {
		if bm.interrupt != nil {
			bm.processor.Stop(InterruptHandlerInvalidLevelBDIStop, 0)
			return false
...
func step6(bm *BankManipulator) bool {
	sbd, ok := bm.processor.findBankDescriptor(bm.sourceBankLevel, bm.sourceBankDescriptorIndex)
	if !ok {
		//	this is worse than the already-posted interrupt... we need to STOP now
		bm.processor.Stop(InterruptHandlerInvalidLevelBDIStop, types.Word36(bm.sourceBankLevel<<15|bm.sourceBankDescriptorIndex))
		return false
	}
	...
	if bm.sourceBankDescriptor.bankType == ExtendedModeBankDescriptor {
		...
		if bm.interrupt != nil {
			bm.processor.Stop(
				...
				types.Word36((bm.sourceBankLevel<<15)|bm.sourceBankDescriptorIndex))
			return false
		} else if bm.instructionType == LBUInstruction &&
			(bm.processor.activityStatePacket.designatorRegister.processorPrivilege > 1) &&
			...
			bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionBDTypeInvalid, bm.sourceBankLevel, bm.sourceBankDescriptorIndex))
			return false
		}
...
func step8(bm *BankManipulator) bool {
	if bm.sourceBankDescriptor.generalFault {
		bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionIndirectGBitSet, bm.sourceBankLevel, bm.sourceBankDescriptorIndex))
		return false
	}

	if (bm.sourceBankLevel == 0) && (bm.sourceBankDescriptorIndex < 32) {
		bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionInvalidSourceLBDI, bm.sourceBankLevel, bm.sourceBankDescriptorIndex))
		return false
	}
	...
	bm.targetBankOffset = bm.sourceBankOffset
	ok := false
	bm.targetBankDescriptor, ok = bm.processor.findBankDescriptor(bm.targetBankLevel, bm.targetBankDescriptorIndex)
	if !ok {
		bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionFatal, bm.targetBankLevel, bm.targetBankDescriptorIndex))
		return false
	}
	...
	if bm.targetBankDescriptor.bankType == BasicModeBankDescriptor {
		...
		if (bm.processor.activityStatePacket.designatorRegister.processorPrivilege > 1) &&
...
func step9(bm *BankManipulator) bool {
	...
	gateBankPerms := bm.sourceBankDescriptor.accessLock.GetEffectivePermissions(
		bm.processor.activityStatePacket.indicatorKeyRegister.accessKey,
	...
	if !gateBankPerms.canEnter {
		bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionEnterAccessDenied, bm.sourceBankLevel, bm.sourceBankDescriptorIndex))
		return false
	}
	...
	if (bm.sourceBankOffset < bm.sourceBankDescriptor.GetLowerLimitNormalized()) ||
		...
		bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionGateBankBoundaryViolation, bm.sourceBankLevel, bm.sourceBankDescriptorIndex))
		return false
	}
	...
	buffer, ok := bm.processor.mainStorage.GetSlice(gateAddr.segment, gateAddr.offset, 8)
	if !ok {
		bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionFatal, bm.sourceBankLevel, bm.sourceBankDescriptorIndex))
		return false
	}
	...
	if (bm.instructionType == GOTOInstruction) ||
		...
		if bm.gate.gotoInhibit {
			bm.processor.PostInterrupt(NewAddressingExceptionInterrupt(AddressingExceptionGateGBitSet, bm.sourceBankLevel, bm.sourceBankDescriptorIndex))
			return false
		}
	}
...
func step10(bm *BankManipulator) bool {
	...
	if bm.instructionType == LAEInstruction {
		...
		bm.baseRegisterIndex = bm.processor.activityStatePacket.currentInstruction.GetA() + 16
		bm.nextStep = 18
	} else if bm.instructionType == LBUInstruction {
		bm.baseRegisterIndex = bm.processor.activityStatePacket.currentInstruction.GetA()
		bm.nextStep = 18
	} else if bm.instructionType == URInstruction {
		...
		sourceModeBasic := bm.processor.activityStatePacket.designatorRegister.BasicModeEnabled
		...
		if destModeBasic {
			if sourceModeBasic {
				...
				if bm.isReturnOperation {
					...
					if bm.processor.activityStatePacket.designatorRegister.BasicModeBaseRegisterSelection {
...
func step11(bm *BankManipulator) bool {
	if bm.transferMode == ExtendedToBasicTransfer {
		bm.processor.SetBaseRegister(0, NewVoidBaseRegister())
		par := bm.processor.activityStatePacket.programAddressRegister
		par.SetLevel(0)
		par.SetBankDescriptorIndex(0)

	} else if bm.transferMode == BasicToBasicTransfer {
		bm.processor.SetBaseRegister(bm.baseRegisterIndex, NewVoidBaseRegister())
	}
...
func step12(bm *BankManipulator) bool {
	if bm.isCallOperation {
		rcsBReg := bm.processor.baseRegisters[RCSBaseRegister]
		if rcsBReg.voidFlag {
			bm.processor.PostInterrupt(NewRCSGenericStackUnderOverflowInterrupt(RCSGenericStackOverflow, RCSBaseRegister, 0))
			return false
		}

		rcsXReg := (*IndexRegister)(bm.processor.generalRegisterSet.GetRegister(RCSIndexRegister))
		framePointer := rcsXReg.GetXM() - 2
		if framePointer < rcsBReg.lowerLimitNormalized {
			bm.processor.PostInterrupt(NewRCSGenericStackUnderOverflowInterrupt(RCSGenericStackOverflow, RCSBaseRegister, framePointer))
			return false
		}

		rtnAddr := bm.processor.activityStatePacket.programAddressRegister.programCounter + 1
		...
		rcsFrame := NewReturnControlStackFrameFromComponents(
			...
			false,
			bValue,
			bm.processor.activityStatePacket.designatorRegister,
			bm.processor.activityStatePacket.indicatorKeyRegister.accessKey)

		offset := framePointer - rcsBReg.lowerLimitNormalized
		buffer := rcsBReg.storage[offset : offset+2]
		rcsFrame.WriteToBuffer(buffer)

		xReg := (*IndexRegister)(bm.processor.generalRegisterSet.GetRegister(RCSIndexRegister))
...
func step13(bm *BankManipulator) bool {
	if bm.isLXJInstruction && (bm.transferMode == BasicToBasicTransfer) {
		parPCNext := bm.processor.activityStatePacket.programAddressRegister.programCounter + 1
		value := TranslateToBasicMode(bm.priorBankLevel, bm.priorBankDescriptorIndex, parPCNext).GetComposite()
		value |= types.Word36(bm.baseRegisterIndex) << 33
		bm.processor.SetExecOrUserXRegister(bm.lxjXRegisterIndex, IndexRegister(value))
	} else if (bm.instructionType == CALLInstruction) && (bm.transferMode == ExtendedToBasicTransfer) {
		bm.processor.SetExecOrUserXRegister(11, 2<<30)
	}
...
func step14(bm *BankManipulator) bool {
	if bm.isCallOperation {
		asp := bm.processor.activityStatePacket
		...
		bm.processor.GetGeneralRegisterSet().SetRegisterValue(X0, types.Word36(value))
	}
...
func step15(bm *BankManipulator) bool {
	if bm.gate != nil {
		...
		if !bm.gate.latentParameter0Inhibit {
			bm.processor.SetExecOrUserRRegister(0, types.Word36(bm.gate.latentParameterValue0))
		}

		if !bm.gate.latentParameter1Inhibit {
			bm.processor.SetExecOrUserRRegister(1, types.Word36(bm.gate.latentParameterValue1))
		}
...
func step16(bm *BankManipulator) bool {
	...
	if bm.interrupt != nil {
		...
		bm.processor.activityStatePacket.ReadFromBuffer(bm.operands)
	} else if bm.instructionType == RTNInstruction {
		bm.processor.activityStatePacket.indicatorKeyRegister.accessKey = bm.returnControlStackFrame.accessKey
		dr := bm.processor.activityStatePacket.designatorRegister
		...
		(bm.transferMode == ExtendedToBasicTransfer) {
		bm.processor.activityStatePacket.designatorRegister.BasicModeEnabled = true
	} else if bm.isLXJInstruction && (bm.transferMode == BasicToExtendedTransfer) {
		bm.processor.activityStatePacket.designatorRegister.BasicModeEnabled = false
	}
...
func step17(bm *BankManipulator) bool {
	if bm.transferMode != NoTransfer {
		bm.processor.SetProgramCounter(bm.targetBankOffset, true)
	}
...
func step18(bm *BankManipulator) bool {
	if bm.baseRegisterIndex == 0 {
		...
		if (bm.interrupt == nil) && (bm.instructionType != URInstruction) {
			bm.processor.activityStatePacket.programAddressRegister.SetLevel(bm.targetBankLevel)
			bm.processor.activityStatePacket.programAddressRegister.SetBankDescriptorIndex(bm.targetBankDescriptorIndex)
		} else if bm.baseRegisterIndex < 16 {
			if bm.targetBankDescriptor == nil {
				bm.processor.activeBaseTable[bm.baseRegisterIndex].SetComposite(0)
			} else {
				offset := uint(0)
				if bm.isLoadInstruction {
					offset = bm.targetBankOffset
				}
				bm.processor.activeBaseTable[bm.baseRegisterIndex].SetBankLevel(bm.targetBankLevel)
				bm.processor.activeBaseTable[bm.baseRegisterIndex].SetBankDescriptorIndex(bm.targetBankDescriptorIndex)
				bm.processor.activeBaseTable[bm.baseRegisterIndex].SetSubsetSpecification(offset)
			}
		}
	}
...
func step19(bm *BankManipulator) bool {
	if bm.targetBankDescriptor == nil {
		...
		bm.processor.baseRegisters[bm.baseRegisterIndex].voidFlag = true
		bm.processor.baseRegisters[bm.baseRegisterIndex].storage = nil
		...
		storage, _ := bm.processor.mainStorage.GetBlock(bm.targetBankDescriptor.baseAddress.segment)
		bm.processor.baseRegisters[bm.baseRegisterIndex].FromBankDescriptorWithSubsetting(
			bm.targetBankDescriptor, bm.targetBankOffset, storage)
	} else {
		//  A normal non-sub-setting base register - make it so.
		storage, _ := bm.processor.mainStorage.GetBlock(bm.targetBankDescriptor.baseAddress.segment)
		bm.processor.baseRegisters[bm.baseRegisterIndex].FromBankDescriptor(bm.targetBankDescriptor, storage)
	}
...
func step20(bm *BankManipulator) bool {
	if (bm.transferMode == BasicToBasicTransfer) || (bm.transferMode == ExtendedToBasicTransfer) {
		bm.processor.FindBasicModeBank(bm.targetBankOffset, true)
	}
...
func step21(bm *BankManipulator) bool {
	if bm.targetBankDescriptor != nil {
		...
		if (bm.transferMode != NoTransfer) && ((bm.gate != nil) || !perms.canEnter) {
			if bm.targetBankDescriptor.bankType == BasicModeBankDescriptor {
				bReg := bm.processor.baseRegisters[bm.baseRegisterIndex]
				relAddr := bm.processor.activityStatePacket.programAddressRegister.programCounter
				interrupt := bReg.CheckAccessLimits(relAddr, false)
				if interrupt != nil {
					interrupt = NewAddressingExceptionInterrupt(AddressingExceptionFatal, bm.targetBankLevel, bm.targetBankDescriptorIndex)
					bm.processor.PostInterrupt(interrupt)
					return false
				}
			}
		}
...
func NewBankManipulator(p *Processor, instructionType int, operand types.Word36) *BankManipulator {
	return &BankManipulator{
		processor:         p,
		interrupt:         nil,
		instructionType:   instructionType,
		operands:          []types.Word36{operand},
		baseRegisterIndex: 0,
	}
}

func NewBankManipulatorForUR(p *Processor, instructionType int, operands []types.Word36) *BankManipulator {
	return &BankManipulator{
		processor:         p,
		interrupt:         nil,
		instructionType:   instructionType,
...
func NewBankManipulatorForLAE(p *Processor, baseRegisterIndex uint, operand types.Word36) *BankManipulator {
	return &BankManipulator{
		processor:         p,
		interrupt:         nil,
...
func NewBankManipulatorForInterrupt(p *Processor, interrupt Interrupt) *BankManipulator {
	return &BankManipulator{
		processor:         p,
		interrupt:         interrupt,
...
func (bm *BankManipulator) process() {
	...
	if bm.interrupt != nil {
		...
		bm.lxjXRegisterIndex = bm.processor.activityStatePacket.currentInstruction.GetA()

		if bm.isLXJInstruction {
			xRegister := bm.processor.GetExecOrUserXRegister(bm.lxjXRegisterIndex)
