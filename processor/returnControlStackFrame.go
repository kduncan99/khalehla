...
func (rcsf *ReturnControlStackFrame) WriteToBuffer(buffer []types.Word36) {
	w0 := uint64(rcsf.bankLevel) << 34
	w0 |= uint64(rcsf.bankDescriptorIndex) << 18
	w0 |= uint64(rcsf.offset)
	buffer[0] = types.Word36(w0)
...
func NewReturnControlStackFrameFromBuffer(source []types.Word36) *ReturnControlStackFrame {
