...
func (addr *BasicModeVirtualAddress) GetComposite() types.Word36 {
	var value types.Word36
	if addr.execFlag {
	...
	value |= types.Word36(addr.bankDescriptorIndex) << 18
	value |= types.Word36(addr.offset)
	return value
}
...
func (addr *ExtendedModeVirtualAddress) GetComposite() types.Word36 {
	value := types.Word36(addr.level) << 33
	value |= types.Word36(addr.bankDescriptorIndex) << 18
	value |= types.Word36(addr.offset)
	return value
}
