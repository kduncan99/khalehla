...
var fileBlockZeroSize = pkg.RawBytesPerBlock[4]
...
func (bd *FileBlockDevice) Open(writeProtected bool, writeThrough bool) DeviceResult {
	...
	bd.geometry.bytesPerBlock = pkg.RawBytesPerBlock[bd.geometry.wordsPerBlock]
...
func CreateFileBlockDevice(fileName string, label string, wordsPerBlock pkg.BlockSize, blockCount pkg.BlockCount, preallocate bool) DeviceResult {
	...
	bd.geometry = BlockGeometry{
		...
		bytesPerBlock:  pkg.RawBytesPerBlock[wordsPerBlock],
	...
	if preallocate {
		offset := int64(int64(blockCount)*int64(pkg.RawBytesPerBlock[wordsPerBlock])) - 1
		b := []byte{0}
