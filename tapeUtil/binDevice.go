...
func (dev *BinDevice) ReadBlock() (data []pkg.Word36, eof bool, err error) {
	data = nil
	eof = false
	err = nil
	if dev.file == nil {
	...
	dataBlock := make([]pkg.Word36, totalWords)
...
func (dev *BinDevice) WriteBlock(buffer []pkg.Word36) error {
	if dev.file == nil {
		return fmt.Errorf("file is not open")
	}
